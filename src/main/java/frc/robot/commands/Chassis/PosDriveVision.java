...
public class PosDriveVision extends Command {
  ...
  public PosDriveVision(double targetx, double targety, double targettheta, double timeout) {
    this.target = new double[] { targetx, targety, targettheta};
    requires(Robot.chassis);
    this.timeout = timeout;
  }

  public PosDriveVision(double targetx, double targety, double targettheta) {
    this.target = new double[] { targetx, targety, targettheta};
    requires(Robot.chassis);
    this.timeout = 100;
  }

  public PosDriveVision(double[] target) {
  ...
  protected boolean isFinished() {
    ...
    this.RobotPos[2] = heading;
    double p = Math.hypot(target[0] - RobotPos[0], target[1] - RobotPos[1]);
    return (Math.abs(p) < 0.05 && Math.abs(heading - target[2]) < 0.05) || timer.get() > this.timeout || !Robot.chassis.isVision;
  }
